...
describe('creation', function()
  spec('can construct', function()
    assert.has_no.errors(function()
      Object()
      Object({})
      Object({ _get_x = function() return 1 end })
    end)
  end)
...
describe('getters', function()
  ...
  spec('prioritizes instance over class properties', function()
    ...
    test_object.instance_default = -1 * test_object.instance_default
    assert.are.equal(-3, test_object.instance_default)
  end)

  spec('has custom getter', function()
    local test_object = Object({ _get_x = function() return 4 end })
...
describe('setters', function()
  ...
  spec('does not override class properties', function()
    local test_object = Object({ class_property = 1 })
    test_object.class_property = -1 * test_object.class_property
    assert.are.equal(-1, test_object.class_property)
...
describe('pubsub', function()
  ...
  spec('can mutate subscriptions while publishing', function()
    ...
    assert.are.equal(1, f1_counter)
    assert.are.equal(1, f2_counter)
    assert.are.equal(1, f3_counter)

    test_object:publish('test')

    assert.are.equal(2, f1_counter)
    assert.are.equal(1, f2_counter)
    assert.are.equal(2, f3_counter)
  end)
